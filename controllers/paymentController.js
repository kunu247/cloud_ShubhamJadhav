...
const createPayment = async (req, res) => {
  try {
    const { payment_type, customer_id, cart_id, product_id, total_amount } =
      req.body;
    const { payment, obj, error } = await createPaymentSql(
      payment_type,
      customer_id,
      cart_id,
      product_id,
    ...
    res
      .status(201)
      .send({ payments: obj, status: 201, msg: "Payment Created" });
  } catch (error) {
    console.log(error);
