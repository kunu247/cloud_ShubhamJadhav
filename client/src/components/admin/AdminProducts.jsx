...
const AdminProducts = () => {
  ...
  useEffect(() => {
    customFetch(`/products`).then((response) => {
  ...
  return (
    <div className="mt-8">
      <div className="flex justify-between">
        <h4 className="capitalize mb-4">total products : {products.length}</h4>
        <button
          ...
          <tbody>
            {products.slice(limit.lower, limit.upper)?.map((product) => {
              ...
              return (
                <tr key={id}>
                  <td>
                    <img src={image} className="w-12 h-12" alt={product_name} />
                  </td>
                  <td>{product_name}</td>
      ...
      <div>
        ...
        <button
          ...
          disabled={limit.upper >= products.length ? true : false}
          onClick={() => {
